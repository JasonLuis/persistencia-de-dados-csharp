...
public class MusicasAPI
{
    ...
    public async Task<ICollection<MusicaResponse>?> GetMusicaPorArtistaAsync(int id)
    {
        return await _httpClient.GetFromJsonAsync<ICollection<MusicaResponse>?>($"musicas/artista/{id}");
    }
}
